...
bun dev                      # start Expo dev server with the MCP bridge
bun ios                      # run on a connected iOS device
bun android                  # run on a connected Android device
bun start                    # run the production-like web preview on port 3000
...
This project already ships `expo-mcp` as a dev dependency and enables the local MCP bridge in `bun dev` and `bun start` via `EXPO_UNSTABLE_MCP_SERVER=1`. After starting or stopping Metro, **reconnect the `expo` MCP server** in Cursor so local tools appear.
...
bun licences:bundle          # regenerate package licenses for release builds
...
- `src/data/package-licenses.json` is a committed empty stub (`{}`) used for local dev and
  type-checking. Release builds run `bun licences:bundle`, which generates
  `src/data/licenses.json` (gitignored) and copies it over the stub before bundling.
  Import package licenses through `@/data/licenses`, not the JSON files directly.
...
- Native compatibility patches and their removal conditions are documented in
  [`patches/README.md`](patches/README.md). Keep each patch file and its
  `patchedDependencies` entry together when committing or removing a patch.
...
- **Don't commit `src/data/licenses.json`.** It is an intermediate build artifact.
