...
| Styling           | **`react-native-unistyles` 2.43** — *not* `StyleSheet.create` for themed UI |
...
- Themes are defined in `src/styles/themes.ts` with `colors`, `margins`, and `radius`
  keys. Use them — do not hard-code colors or magic numbers.
...
- Plain `StyleSheet.create` is fine for static, non-themed wrappers (e.g. flex containers);
  anything color-, size-, or radius-related must go through Unistyles.
...
- **Don't read or write `localStorage` / `SecureStore` directly.** Use the helpers in
  `@/utils/storage`.
- **Don't hard-code colors or pixel values.** Pull them from the Unistyles theme.
