...
export async function getMobilityEntries(kind, station) {
  if (kind === 'bus') {
    ...
    return [
      ...stations.charging.map((x) => data.find((y) => x.id === y.id)).filter((x) => !!x),
  ...
  if (kind === 'bus') {
    return (
      <>
        ...
        <div className={styles.mobilityTime}>{formatRelativeMinutes(new Date(item.time))}</div>
      </>
    )
  } else if (kind === 'train') {
    return (
    ...
    return (
      <>
        ...
        <div className={styles.mobilityTime}>
          {typeof item.available === 'number' ? item.available + ' frei' : 'n/a'}
        </div>
      </>
    )
  } else if (kind === 'charging') {
