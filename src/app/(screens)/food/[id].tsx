...
export default function FoodDetail(): React.JSX.Element {
			...
			items:
				foodData?.flags?.map((flag: string) => ({
					...
					icon: preferencesSelection.includes(flag)
						...
						: undefined,
					hideChevron: true,
					iconColor: toColor(successColor),
					onPress: () => {
						itemAlert(flag, 'flag')
					}
				})) ?? [],
				...
				(foodData?.allergens ?? [])
					...
					.map((allergen: string) => ({
						...
						icon: allergenSelection.includes(allergen)
							...
							: undefined,
						hideChevron: true,
						iconColor: toColor(notificationColor),
						onPress: () => {
			...
			items: [
				{
					...
					value: humanLocation,
					onPress: handlePress,
					textColor: locationExists ? toColor(primaryColor) : undefined,
	...
	return (
		...
		>
			...
			/>
			<View className="items-start flex-row justify-between pb-1.5">
				<Text
			...
			<View className="flex-row gap-2 mb-4 items-center">
				{mealWithDate?.date && (
					<View
						className="flex-row items-center gap-1.5 bg-card-sheet px-3 py-1.5 rounded-md border border-border"
			...
			{isPriceAvailable && (
				<View className="flex-row gap-2 mb-4">
					<View
						className={`flex-1 rounded-md p-3 items-center border ${
							userKind === USER_STUDENT
								? 'bg-primary-background border-primary'
								: 'bg-card-sheet border-border'
						}`}
						style={hairlineBorder}
					>
			...
			<Pressable onPress={triggerWiggle}>
				<View className="self-center mb-bottom-safe mt-5 px-1">
					<View
						className="items-center self-center bg-card-sheet rounded-md flex-row gap-4 px-3.5 py-2 w-full border border-border"
						style={hairlineBorder}
					>
						<Animated.View style={wiggleIconAnimatedStyle}>
							<PlatformIcon
								...
								style={{ color: toColor(warningColor) }}
							/>
						</Animated.View>
						<Text className="text-label flex-1 shrink text-[11px]">
