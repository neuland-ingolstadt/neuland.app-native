			...
			<ContextMenu
				...
				actions={[
						...
						: []),
					{
						...
						systemIcon: Platform.OS === 'ios' ? 'paintpalette' : undefined
					},
					{
						title: t('navigation.about'),
						systemIcon: Platform.OS === 'ios' ? 'info.circle' : undefined
					},
					...(userKind !== 'guest'
						? [
								{
									title: 'Logout',
									systemIcon:
										Platform.OS === 'ios' ? 'person.fill.xmark' : undefined,
									destructive: true
								}
							]
						: []),
				...
				onPress={(e) => {
					if (e.nativeEvent.name === t('navigation.profile')) {
						...
						e.nativeEvent.name === t('menu.guest.title', { ns: 'settings' })
					) {
